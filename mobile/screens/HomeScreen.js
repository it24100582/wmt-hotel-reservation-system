    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.hero}>
        <View style={styles.heroOverlay}>
          <Text style={styles.heroTag}>⭐ Luxury Hotel & Resort</Text>
          ...
          <View style={styles.heroButtons}>
            ...
            >
              <Text style={styles.btnOutlineText}>{user ? `Hi, ${user.name.split(' ')[0]}` : 'Sign In'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
        ...
        {[['★★★★★', '5-Star Rating'], ['4,200+', 'Happy Guests'], ['25 Yrs', 'Experience']].map(([val, label], i) => (
          <React.Fragment key={label}>
            ...
            <View style={styles.ratingItem}>
              <Text style={i === 0 ? styles.ratingStars : styles.ratingNumber}>{val}</Text>
      ...
      <View style={styles.featuresStrip}>
        {[['🏖️', 'Beach Access'], ['🍽️', 'Free Breakfast'], ['🏊', 'Pool & Spa'], ['🚗', 'Free Parking']].map(([icon, label]) => (
      ...
      <View style={styles.section}>
        ...
        {loadingRooms ? (
          ...
          <Text style={{ color: 'rgba(255,255,255,0.4)', textAlign: 'center', paddingVertical: 20 }}>
            No rooms available right now.
          </Text>
        ) : (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.roomsScroll}>
            {featuredRooms.map((room) => {
              const imgUri = room.imageUrl
              ...
              return (
                ...
                >
                  {imgUri ? (
                    <Image source={{ uri: imgUri }} style={styles.roomCardImage} resizeMode="cover" />
                  ) : (
                    <View style={styles.roomImagePlaceholder}>
                      <Text style={styles.roomImageIcon}>🛏️</Text>
                    </View>
                  )}
                  <View style={styles.roomInfo}>
                    <Text style={styles.roomType}>{room.roomType}</Text>
                    <Text style={styles.roomName}>Room #{room.roomNumber}</Text>
                    <Text style={styles.roomPrice}>Rs. {room.pricePerMonth?.toLocaleString()}/month</Text>
      ...
      {/* ABOUT SECTION */}
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Welcome to WMT</Text>
      ...
      <View style={styles.contactStrip}>
        ...
        <Text style={styles.contactSubtitle}>📞 1300-736-198</Text>
        <TouchableOpacity style={styles.btnGold} onPress={() => navigation.navigate('Rooms')}>
      ...
      <View style={styles.footer}>
        <Text style={styles.footerLogo}>WMT</Text>
        <Text style={styles.footerText}>© 2026 WMT. All rights reserved.</Text>
      </View>
    </ScrollView>
  );
};
...
const styles = StyleSheet.create({
  ...
  heroTitle: { color: '#fff', fontSize: 32, fontWeight: 'bold', textAlign: 'center', marginBottom: 15, fontFamily: 'serif' },
  heroSubtitle: { color: 'rgba(255,255,255,0.7)', fontSize: 14, textAlign: 'center', marginBottom: 25, lineHeight: 22 },
  ...
  btnOutline: { borderWidth: 2, borderColor: '#fff', paddingVertical: 14, paddingHorizontal: 22, borderRadius: 8 },
  btnOutlineText: { color: '#fff', fontSize: 15, fontWeight: 'bold' },
  ratingStrip: { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', backgroundColor: '#1a1a2e', paddingVertical: 20, borderTopWidth: 1, borderTopColor: 'rgba(212,175,55,0.3)' },
  ratingItem: { alignItems: 'center' },
  ratingStars: { color: '#d4af37', fontSize: 16 },
  ...
  featuresStrip: { flexDirection: 'row', justifyContent: 'space-around', backgroundColor: '#d4af37', paddingVertical: 15 },
  featureItem: { alignItems: 'center' },
  featureIcon: { fontSize: 24, marginBottom: 5 },
  featureText: { color: '#1a1a2e', fontSize: 11, fontWeight: '600' },
  section: { padding: 20 },
  sectionTitle: { color: '#fff', fontSize: 24, fontWeight: 'bold', textAlign: 'center', marginBottom: 8, fontFamily: 'serif' },
  sectionSubtitle: { color: 'rgba(255,255,255,0.6)', fontSize: 14, textAlign: 'center', marginBottom: 20, lineHeight: 22 },
  roomsScroll: { marginTop: 10 },
  roomCard: { width: 240, backgroundColor: '#1a1a2e', borderRadius: 12, marginRight: 15, overflow: 'hidden' },
  roomCardImage: { width: '100%', height: 140 },
  roomImagePlaceholder: { height: 140, backgroundColor: '#2a2a4e', justifyContent: 'center', alignItems: 'center' },
  roomImageIcon: { fontSize: 45 },
  roomInfo: { padding: 12 },
  ...
  amenityTag: { backgroundColor: 'rgba(212,175,55,0.2)', color: '#d4af37', fontSize: 9, paddingHorizontal: 6, paddingVertical: 3, borderRadius: 4 },
