...
const ConfirmationScreen = () => {
  ...
  if (!booking) {
    return <View style={styles.center}><Text style={styles.errText}>No booking data.</Text></View>;
  }

  const fmt = (dateStr) => new Date(dateStr).toLocaleDateString('en-US', {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.rowValue}>#{booking._id?.slice(-8).toUpperCase()}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  errText: { color: '#fff' },
  successBanner: { backgroundColor: '#1a1a2e', padding: 30, alignItems: 'center', borderBottomWidth: 2, borderBottomColor: '#d4af37' },
  ...
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.08)' },
  ...
  infoBox: { flexDirection: 'row', backgroundColor: 'rgba(245,158,11,0.1)', margin: 20, marginTop: 0, borderRadius: 8, padding: 15, borderWidth: 1, borderColor: 'rgba(245,158,11,0.3)' },
  ...
  btnOutline: { borderWidth: 2, borderColor: 'rgba(255,255,255,0.3)', padding: 16, borderRadius: 10, alignItems: 'center' },
